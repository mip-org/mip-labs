    ...
    methods (Static)
        function configureBasicConsoleLogging(style)
            ...
            if ~isConfiguredAlready
                ...
                % --- mip-org/labs channel patch ---
                % On recent MATLAB (log4j-over-reload4j backends),
                % BasicConfigurator.configure() may leave the root logger with
                % no appenders, so getAllAppenders() returns a NullEnumeration
                % and nextElement() throws NoSuchElementException. Upstream
                % v0.3.0 calls nextElement() unconditionally, which crashes
                % library initialization (and thus every export entry point) on
                % those MATLAB releases. Guard with hasMoreElements so
                % configuration degrades gracefully (the console pattern is
                % simply left at log4j's default) instead of aborting.
                appendersEnum = rootLogger.getAllAppenders();
                if appendersEnum.hasMoreElements
                    rootAppender = appendersEnum.nextElement();
