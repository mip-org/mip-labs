...
% The fan-out is real on MSVC too (SSE2/AVX/AVX2/AVX512 via /arch:, 24 TUs); it
...
elseif ispc
    ...
    % -T ClangCL selects the clang-cl that ships inside Visual Studio 2022, so no
    % PATH or vcvarsall setup is needed. cl.exe is not an option here: it does not
    % finish optimizing one dispatch TU in 40 minutes and the multiarch fan-out is
    % 24 of them, which is what pushed the 0.0.3 Windows build past the 6-hour job
    % cap. clang-cl builds the same target in about 3 minutes and emits the same
    % MSVC ABI. If configure fails, install the "C++ Clang tools for Windows"
    % component of Visual Studio 2022.
    genArg = ' -G "Visual Studio 17 2022" -A x64 -T ClangCL';
