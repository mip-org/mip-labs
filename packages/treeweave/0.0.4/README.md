...
Tag-tracked at upstream `v0.0.4`.
...
| `windows_x86_64` | clang-cl (VS 2022, `-T ClangCL`) | C++20; SSE2 baseline, dispatch ladder up to AVX-512 via `/arch:` flags. cl.exe cannot optimize the 24 dispatch TUs in a workable time; clang-cl emits the same MSVC ABI in minutes. |
