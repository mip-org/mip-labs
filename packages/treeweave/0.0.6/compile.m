...
% The fan-out is real on MSVC too (SSE2/AVX/AVX2/AVX512 via /arch:) and
% collapses to single-arch only on Apple-arm64, so the shipped binary loads on
% any CPU of the family yet still uses AVX2/AVX-512 (or NEON) where present.
defs = { ...
    ...
    '-DTREEWEAVE_VERIFY_RUNGS=ON', ...   % gate: rung objects must share no divergent symbol
...
elseif ispc
    ...
    % No -T: the default v143 toolset is cl.exe, the compiler MathWorks supports
    % for MEX on Windows.
    genArg = ' -G "Visual Studio 17 2022" -A x64';
    % COFF has no hidden visibility, so upstream renames every ISA rung's defined
    % external symbols and needs llvm-nm and llvm-objcopy at configure time. Add
    % the usual install locations to PATH; upstream fails configure with an
    % explicit message if neither carries them.
    add_llvm_bin_to_path();
    % No bison/flex/mwrap: the mwrap gateway is checked in upstream, so the build
    % just compiles it.
else
...
function p = which_on_path(exeName)
% Empty when exeName is not on PATH. `where` is the Windows equivalent of which.
[st, out] = system(sprintf('where %s', exeName));
if st == 0
