...
Tag-tracked at upstream `v0.0.6`.
...
[`bindings/matlab/README.md`](https://github.com/DiamonDinoia/treeweave/blob/v0.0.3/bindings/matlab/README.md)
...
| `windows_x86_64` | cl.exe (VS 2022, default v143 toolset) | C++20; SSE2 baseline, dispatch ladder up to AVX-512 via `/arch:` flags. COFF has no hidden visibility, so upstream gives every rung private symbol names. Needs `llvm-nm` and `llvm-objcopy` at configure time. |
